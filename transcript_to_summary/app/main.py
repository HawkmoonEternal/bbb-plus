...
from transcript_processor import TranscriptProcessor, Block, Section, SummaryResponse
import time
from typing import List

def render_blocks(blocks: List[Block]) -> str:
    """Convert a list of blocks into readable text."""
    lines = []
...
def render_section(title: str, section: Section) -> str:
    """Render a section with title and blocks."""
    output = [f"\n\n=== {title.upper()} ===\n"]
    output.append(render_blocks(section['blocks']))
    return "\n".join(output)


def summary_to_text(summary: SummaryResponse) -> str:
    """Convert full MeetingSummaryResponse into human-readable text."""
    parts = []

    # Title
    parts.append(f"# MEETING: {summary['MeetingName']}\n")

    # People
    parts.append("\n=== PEOPLE ===\n")
    parts.append(render_blocks(summary['People']['blocks']))

    # Sections
    parts.append(render_section("Session Summary", summary['SessionSummary']))
    parts.append(render_section("Critical Deadlines", summary['CriticalDeadlines']))
    parts.append(render_section("Key Items & Decisions", summary['KeyItemsDecisions']))
    parts.append(render_section("Immediate Action Items", summary['ImmediateActionItems']))
    parts.append(render_section("Next Steps", summary['NextSteps']))

    # Nested meeting notes
    #parts.append("\n\n=== MEETING NOTES ===\n")
    #for section in summary['MeetingNotes']['sections']:
    #    parts.append(render_section(section['title'], section))
...
async def process_transcript_api(
    ...
    try:
        ...
        if transcript.text == None:
            ...
            transcript.text = read_file("/input/"+transcript.meeting_id+".txt")
...
async def get_summary(meeting_id: str):
    ...
    try:
        ...
        if status == "failed":
            ...
            with open("/output/"+meeting_id+".txt", "w") as f:
                f.write(summary_to_text(summary_data))
